...
use crate::{
    AdapterRequest, AuthenticationError, AuthenticationRequest, HttpAdapter, ServerConfiguration,
...
impl LocalHttpServer {
    ...
    pub fn start(
        ...
        adapters: Vec<Arc<dyn HttpAdapter>>,
...
impl ServerState {
    async fn handle(&self, mut request: Request<Incoming>, peer: SocketAddr) -> Response<Bytes> {
        ...
        secure_response(
            adapter
                .handle(AdapterRequest::new(request, principal, peer))
                .await,
        )
    }
}
