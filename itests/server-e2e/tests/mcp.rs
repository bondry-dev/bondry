...
use bondry_core::{
    ...
    HandlerError, HandlerErrorCode, Invocation, InvocationId, InvocationIdGenerationError,
    InvocationIdGenerator, Principal, PrincipalId, PrincipalKind,
};
use bondry_http_server::{Authentication, LocalHttpServer, MountedProtocol, ServerConfiguration};
use bondry_mcp_proto::{LATEST_PROTOCOL_VERSION, MCP_PATH, McpAdapter, McpServerInfo};
...
fn server_info() -> Result<McpServerInfo, bondry_mcp_proto::McpServerInfoError> {
...
fn start_server(
    ...
    Ok(LocalHttpServer::start(
        ...
        vec![MountedProtocol::Mcp(adapter)],
    )?)
}
